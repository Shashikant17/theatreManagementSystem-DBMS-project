...
app.config['MYSQL_HOST'] = '13.126.82.38'
...
def Moviedeletion():
    ...
    if request.method == 'POST' and 'to_movie_name' in request.form and 'from_movie_name' in request.form and 'duration' in request.form and 'movie_type' in request.form and 'genre' in request.form:
        ...
        msg = 'Please fill out the form!!!'
    return render_template('moviedeletion.html', msg=msg,theatre=theatre)
...
def theatreinsert():
    ...
    if request.method == 'POST' and 'hall_id' in request.form and 'hall_name' in request.form and 'location' in request.form and 'price' in request.form :
        ...
        msg = 'Please fill out the form!'
    return render_template('theatreinsert.html', msg=msg,theatre=theatre)
...
def movie_name(var):
    ...
    movieName = spill2[0]
    cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
    cursor.execute('select distinct movie_type from movies where movie_name=%s',(movieName,))
    mt=cursor.fetchall()
    movietype=mt[0]['movie_type']
...
def details():
    ...
    data = cursor.fetchall()
    '''
    if request.method == 'POST' and 'Seat' in request.form:
        s = request.form['Seat']
        seat=int(s)
    
    print("seat",seat)
    '''
    return render_template('2.html',data=data,theatre=theatre)
...
def choosetheatre():
    if request.method == 'POST' and 'billing_first_name' in request.form and 'billing_last_name' in request.form and 'noofperson' in request.form and 'Seat' in request.form:
...
def home():
    ...
    if 'loggedin' in session:
        ...
        #return render_template('Billing_details')
...
def mailing():
    ...
    m.mail(username,movieName,T_name,Primary_location,time,Price,email,seat_name,today)
    return "0"
...
def forgot():
    ...
    if request.method == 'POST' and 'email' in request.form and 'Password' in request.form:
        ...
        account = cursor.fetchone()
        if account:
            cursor.execute('update user set password=%s where email_id=%s',(password,email,))
            mysql.connection.commit()
            msg="Successfully updated!!"
        else:
            msg="There is no account available with this email!! " 
